...
function UnitCases() {
  ...
  const handleViewDiagnosis = async () => {
    try {
      //const authToken = localStorage.getItem('authToken');
      const authToken = 'eyJhbGciOiJIUzM4NCJ9.eyJzdWIiOiJhbnVqYWc3OEBnbWFpbC5jb20iLCJpYXQiOjE3MTUxMDE1MTksImV4cCI6MTcxNTEzNzUxOX0.0IwFoCUqEHvul9Lm_164yTU07MvOc2bCDuLNNqqZ7BqZb7ASpyeVvMMVl0PYcS-t';
      const response = await axios.get(`http://localhost:9191/api/v1/patient/get-diagnosis?caseId=1`, {
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${authToken}`
        },
      ...
      // Create blob URL to view the PDF
  ...
  const handleDownloadDiagnosis = async () => {
    try {
      ...
      setError('An error occurred while downloading diagnosis');
    }
  };
        ...
        >
          ...
          <div className="right-half">
            ...
            <div className="report-box">
              <h2>Final Reports</h2>
              <div className="button-container">
                <StyledButton variant="contained" onClick={handleViewDiagnosis}>View Diagnosis</StyledButton>
                <StyledButton variant="contained" onClick={handleDownloadDiagnosis}>Download</StyledButton>
              </div>
            </div>
          </div>
        </Box>
